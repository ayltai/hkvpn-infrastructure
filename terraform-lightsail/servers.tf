...
resource "null_resource" "exec" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ansible-playbook -u ${var.username} --private-key ${var.private_key} --vault-password-file ${var.vault_password_file} --extra-vars "ip_address=${aws_lightsail_static_ip.hkvpn.ip_address} dist_upgrade=false" -i inventory.ini ../ansible/playbook.yml
    EOT
  }
}
